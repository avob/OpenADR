...
public class Oadr20aVTNEiEventControllerTest {
     ...
     * oadrDisributeEvent eiEventSignal, UID must be expressed as an interval
     * number with a base of 0 and an increment of 1 for each subsequent
     * interval
     * 
     * 7:VTN, EiEvent Service, oadrDistributeEvent Payload The
     * oadrDistibuteEvent eiEvent object must contain only one event signal with
     * a signalName of “simple”.
     * 
     * 14: VTN, EiEvent Service, oadrDistributeEvent Payload The currentValue
    ...
    public void testPullOadrRequestEventActiveSuccessCase() throws Exception {
        ...
        eventActive.setComaSeparatedTargetedVenUsername(VEN);
        ...
        eventCanceled.setComaSeparatedTargetedVenUsername(VEN);
     ...
     * 
     * 4: VTN, EiEvent Service, oadrDistributeEvent Payload - New events start
     * with a modificationNumber of 0.
     * 
     * 5: VTN, EiEvent Service, oadrDistributeEvent Payload - Each modification
     * of the oadrDisributeEvent eiEvent object, excluding createdDateTime,
     * eventStatus, and currentValue, cause the modificationNumber to increment
     * by 1 and an updated event sent to the VEN. Exception: An eventStatus
     * change to modification number to increment by 1 "cancelled" shall cause
     * the modification number to increment by 1
     * 
     ...
     * recognize an async eiCreatedEvent optOut for a previously acknowledged
     * event.
     * 
     * 
     * 
    ...
    public void testScenario1() throws Exception {
        ...
        dto.setComaSeparatedTargetedVenUsername(venId);
        ...
        // eventResponse.setRequestID("");
        // QualifiedEventIDType qualifiedEventID = new QualifiedEventIDType();
        // qualifiedEventID.setEventID(eventId.toString());
        // qualifiedEventID.setModificationNumber(modificationNumber);
     ...
     * 
     * 4) user creates two 'active'/'canceled' DR events with oadr state
     * 'active'
     * 
     * 5) user creates two 'active'/'canceled' DR events with oadr state
     * 'completed'
     * 
