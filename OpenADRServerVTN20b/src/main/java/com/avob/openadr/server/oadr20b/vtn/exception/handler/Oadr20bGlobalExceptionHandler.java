...
public class Oadr20bGlobalExceptionHandler {
	...
	public String genericException(Oadr20bGenericException exception, Logger logger) {
		logger.warn(exception.getMessage());
		try {
			if (exception.isSigned()) {
				return xmlSignatureService.sign(exception.getResponse());
			} else {
				return jaxbContext.marshalRoot(exception.getResponse());
			}

		} catch (Oadr20bMarshalException e) {
			logger.error("Can't marshall error response", e);
		} catch (Oadr20bXMLSignatureException e) {
			logger.error("Can't sign error response", e);
		}
		return null;
	}
