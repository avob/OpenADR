...
//	private List<DemandResponseEventCreateDto> eventTemplate = new ArrayList<>();
	...
	public void init() {
		...
		marketContexts.forEach(marketContext -> {
			 ...
			 * DemandResponseEventActivePeriodDto activePeriod = new
			 * DemandResponseEventActivePeriodDto();
			 * activePeriod.setDuration(marketContext.getActivePeriod().getDuration());
			 * activePeriod.setNotificationDuration(marketContext.getActivePeriod().
			 * getNotificationDuration());
			 * activePeriod.setRampUpDuration(marketContext.getActivePeriod().
			 * getRampUpDuration());
			 * activePeriod.setRecoveryDuration(marketContext.getActivePeriod().
			 * getRecoveryDuration());
			 * activePeriod.setToleranceDuration(marketContext.getActivePeriod().
			 * getToleranceDuration()); eventRead.setActivePeriod(activePeriod);
			 * 
			 * if (marketContext.getBaseline() != null) { DemandResponseEventBaselineDto
			 * baseline = new DemandResponseEventBaselineDto();
			 ...
			 * baseline.setIntervals(marketContext.getBaseline().getIntervals().stream().map
			 * (i -> { DemandResponseEventSignalIntervalDto eventInterval = new
			 * DemandResponseEventSignalIntervalDto();
			 * eventInterval.setDuration(i.getDuration());
			 * eventInterval.setValue(i.getValue()); return eventInterval;
			 * }).collect(Collectors.toList())); eventRead.setBaseline(baseline); }
			 * 
			 * DemandResponseEventDescriptorDto descriptor = new
			 * DemandResponseEventDescriptorDto();
			 * descriptor.setMarketContext(marketContext.getName());
			 * descriptor.setOadrProfile(
			 * OadrProfileEnum.fromValue(marketContext.getDescriptor().getOadrProfile().
			 * getValue()));
			 * descriptor.setPriority(marketContext.getDescriptor().getPriority());
			 * descriptor.setResponseRequired(
			 * ResponseRequiredEnum.fromValue(marketContext.getDescriptor().
			 * getResponseRequired().getValue())); descriptor.setTestEvent(false);
			 ...
			 * DemandResponseEventSignalDto eventSignal = new
			 * DemandResponseEventSignalDto(); eventSignal.setItemBase(s.getItemBase());
			 * eventSignal.setIntervals(s.getIntervals().stream().map(i -> {
			 * DemandResponseEventSignalIntervalDto eventInterval = new
			 * DemandResponseEventSignalIntervalDto();
			 * eventInterval.setDuration(i.getDuration());
			 * eventInterval.setValue(i.getValue()); return eventInterval;
			 * }).collect(Collectors.toList()));
			 * eventSignal.setSignalName(s.getSignalName()); eventSignal.setSignalType(
			 * DemandResponseEventSignalDto.SignalTypeEnum.fromValue(s.getSignalType().
			 * getValue())); eventSignal.setTargets(s.getTargets());
			 * 
			 * return eventSignal; }).collect(Collectors.toList()));
			 * 
			 * eventRead.setTargets(marketContext.getTargets());
			 * 
			 * eventTemplate.add(eventRead); DemandResponseEventFilter filter = new
			 * DemandResponseEventFilter(); filter.setType(TypeEnum.MARKET_CONTEXT);
			 * filter.setValue(marketContext.getName());
			 * 
			 * OffsetDateTime now = OffsetDateTime.now(); OffsetDateTime truncatedTo =
			 * now.truncatedTo(ChronoUnit.HOURS); Long start = truncatedTo.toEpochSecond() *
			 * 1000; Long end = truncatedTo.plusHours(6).toEpochSecond() * 1000; Integer
			 * totalCount; int page = 0; List<DemandResponseEventReadDto> events = new
			 * ArrayList<>(); try {
			 * 
			 * do {
			 * 
			 * DemandResponseEventSearchDto demandResponseEventSearch = new
			 * DemandResponseEventSearchDto();
			 * demandResponseEventSearch.filters(Arrays.asList(filter));
			 * ApiResponse<List<DemandResponseEventReadDto>> response =
			 * demandResponseControllerApi
			 * .searchUsingPOSTWithHttpInfo(demandResponseEventSearch, end, page, null,
			 * start); totalCount =
			 * Integer.valueOf(response.getHeaders().get(X_TOTAL_COUNT).get(0)); page++;
			 * events.addAll(response.getData()); } while (events.size() < totalCount);
			 * 
			 * events.forEach(event -> { LOGGER.info(String.format("%s",
			 * String.valueOf(event.getId()))); });
			 * 
			 * } catch (ApiException e) { LOGGER.error("Market context: " +
			 * marketContext.getName() + " events can't be retrieved", e); return; }
			 * 
