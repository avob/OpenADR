...
export const fetchRequestedReport = (venId, reportSpecifierId) => {
  return (dispatch, getState) => {
      dispatch({
        type    : FETCH_REQUESTEDREPORT
        , swagger: function(api) {
          api.apis["oadr-20b-ven-controller"].viewReportRequestUsingGET({venID: venId, reportSpecifierId:reportSpecifierId}, {responseContentType: 'application/json'}, function(data){
            var reports = JSON.parse(data.data);
            dispatch({
              type    : FETCH_REQUESTEDREPORT_SUCCESS
              , payload: reports
              , notification: notifySuccess("Report requests have been retrieved")
            });
          }, function(err){
            dispatch({
              type    : FETCH_REQUESTEDREPORT_ERROR
              , payload: err
              , notification: notifyError("Report requests have not been retrieved")
            });
          })
